...
const hasValidCoordinatePairs = (shape, coords) => {
    if (shape === 'rect') {
        if (coords.length !== 4) {
            // Rectangles must have exactly 4 coordinates
            return false;
        }
        ...
        if (coords.length !== 3) {
            // Circles must have exactly 3 coordinates
            return false;
        }

        const [x, y, radian] = coords;

        if (isNaN(x) || isNaN(y) || isNaN(radian) || radian <= 0) {
            // x, y, and radian must be numbers and radian must be positive
            return false;
        }
    } else if (shape === 'poly') {
        if (coords.length % 2 !== 0) {
            // Polygons must have an even number of coordinates
            return false;
        }
        if (coords.length < 6) {
            // Rectangles must have exactly 6 coordinates
            return false;
        }
        ...
        for (let i = 0; i < coords.length; i += 2) {
            ...
            if (isNaN(x) || isNaN(y)) {
                // If either x or y is not a number, it's not a valid coordinate pair
                return false;
            }
        }
    } else {
